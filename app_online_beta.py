...
with st.expander("🎯 用途和意义 Purpose & Value", expanded=False):
    ...
    **1. 生成最优强化策略，降低长期强化成本**  
    玩家可以根据装备类型或自行设定灵魂宝石成功率，计算从 +0 强化到目标等级时，每个阶段应优先使用祝福宝石还是灵魂宝石。对于经常或大量进行强化的玩家，该工具可以帮助减少无效尝试，在长期强化过程中降低平均宝石消耗。
    ...
    **2. 评估 +0 装备与高等级装备之间的理论价值差异**  
    ...
    但需要注意，高等级装备的实际出售价格通常不完全等于理论成本。很多高等级装备出售时，卖方往往是不再需要该装备，或希望尽快换取现金/宝石，因此市场常处于买方更有议价能力的状态。相反，+0 装备往往来自野外击杀怪物、打宝掉落或基础交易，卖方希望获得相对合理回报，市场状态更接近对等交易，某些稀缺底材甚至可能偏向卖方市场。因此，+7 或 +9 装备的实际挂售价格通常可能低于其理论强化成本。

    **3. 判断“自己强化”还是“直接购买”更划算**  
    如果高等级装备的市场价格明显低于本工具计算出的期望强化成本，直接购买可能更理性；如果市场上成品价格过高，或者目标装备稀缺，则自行强化可能更有优势。该工具可以为这类决策提供量化依据。

    **4. 分析不同服务器经济环境下的策略变化**  
    不同区服中祝福、灵魂和其他资源的价格比例可能差异很大。通过调整祝福相对价值，玩家可以观察最优策略如何变化，判断当前服务器中祝福宝石是否“值得用”，以及在哪些等级更值得使用。

    **5. 评估不同装备类型的强化风险**  
    不同装备类型可能对应不同的灵魂成功率。成功率较低时，祝福宝石的确定性价值更高；成功率较高时，灵魂宝石的低成本优势更明显。本工具可以帮助玩家理解不同装备类型下的风险与成本差异。

    **6. 为交易、囤货和资源配置提供参考**  
    玩家可以根据模型结果判断祝福宝石和灵魂宝石的相对价值是否合理。如果某一类宝石价格偏低，可能意味着其在强化过程中的实际价值被低估；如果价格偏高，则可能需要减少使用或等待市场回落。对于长期玩家、商人型玩家或公会资源管理者，该工具可以辅助制定更合理的资源配置方案。

    ---

    ### What can this tool be used for?

    This tool is not just a calculator for estimating how many gems are needed. It is designed to identify better resource allocation strategies under different success rates and gem price conditions. It is especially useful for players who frequently upgrade items, perform bulk upgrades, or want a more rational basis for item valuation.

    **1. Generate optimal upgrade strategies and reduce long-term upgrade cost**  
    Players can select an item type or manually set the Soul Gem success rate. The tool then calculates whether Bless or Soul should be used at each upgrade stage. For frequent or large-scale upgrades, this helps reduce inefficient attempts and lower the long-run expected gem cost.

    **2. Estimate the theoretical value gap between +0 and high-level items**  
    By using the relative market price between Bless and Soul Gems, the tool can estimate the expected cost of upgrading from +0 to +7 or +9. This provides a reference for item pricing. For example, the theoretical value of a +9 item should consider both the base item value and the expected gem cost required for upgrading.

    However, actual market prices may deviate from theoretical cost. High-level items are often sold because the seller no longer needs them or wants quick liquidity, which usually creates a buyer-favored market. In contrast, +0 items are often obtained from monster drops or basic farming, and sellers may expect a more balanced return; rare base items can even lean toward a seller-favored market. Therefore, the actual listing price of +7 or +9 items is often lower than their theoretical upgrade cost.

    **3. Decide whether to upgrade by yourself or buy directly**  
    If the market price of a high-level item is much lower than the expected upgrade cost calculated by this tool, buying directly may be more rational. If finished items are overpriced or unavailable, upgrading by yourself may be preferable.

    **4. Analyze strategy changes under different server economies**  
    Gem prices vary across servers. By adjusting the Bless relative cost, players can observe how the optimal strategy changes and determine whether Bless Gems are worth using in their specific server economy.

    **5. Compare upgrade risk across item types**  
    Different item types may have different Soul success rates. When the success rate is low, the certainty of Bless Gems becomes more valuable; when the success rate is high, the low cost of Soul Gems becomes more attractive.

    **6. Support trading, hoarding, and resource allocation decisions**  
    The model can help players judge whether Bless or Soul Gems are currently undervalued or overvalued. For long-term players, market traders, or guild resource managers, it provides a quantitative reference for planning gem usage and inventory decisions.
    """)
