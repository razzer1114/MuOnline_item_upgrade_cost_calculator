...
- 祝福与灵魂的价格比例可能非常接近；
- 某些新区甚至会出现价格倒挂；
- 老玩家虽然都知道极端情况下肯定祝福更好用，但“常规”和“极端”之间，到底什么时候该用祝福？
...
其实是一个需要结合概率、市场价格与长期期望成本共同分析的问题。

而本工具的目标，就是将这些原本依赖经验判断的问题，转化为客观、可验证、可复现的理论模型。
        """)
...
Under different server economies and market environments:
...
- veteran players may know that Bless Gems are safer in extreme cases, but still struggle to determine when Bless Gems are actually worth using.

Similarly, when both +0 base items and +7 / +9 finished items coexist in the market, players often rely on intuition or community experience to decide:
...
### Common misconceptions in upgrading

One common player perspective repeatedly appeared during development:

> “Soul Gems drop more often, so using Soul Gems doesn’t feel expensive.”

However, this viewpoint often ignores several important facts:

- Soul Gems and Bless Gems are fundamentally interchangeable through the market;
- upgrade failures create repeated consumption and long-term cumulative cost.

Although Soul Gems may appear cheaper per attempt, failures can cause downgrades and repeated upgrades, leading to total resource consumption far higher than intuitive expectations.

As a result, the truly important question is not:

> “What gem was used in a single attempt?”
...
> “Use only Soul Gems for lucky items, and use Bless only up to +2 for non-lucky items.”

However, under different servers and market environments:

- the relative value between Bless and Soul Gems may vary dramatically;
- Soul success rates may differ across equipment types;
- the same experience-based strategy may not apply universally.

Therefore:

> “When should Bless Gems actually be used?”

is fundamentally a problem involving probability, market value, and long-term expected cost.

The purpose of this project is to convert these experience-driven questions into objective, reproducible, and theoretically grounded models.

---

### Practical applications

- Generate optimal upgrade strategies
- Reduce long-term gem consumption
- Evaluate theoretical item value
- Compare self-upgrading vs direct purchase
- Analyze market-driven strategy changes
- Support trading and resource allocation decisions
        """)
