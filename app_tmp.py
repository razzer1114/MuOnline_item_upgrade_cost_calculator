...
with st.expander("🎯 用途和意义 Purpose & Value", expanded=False):
    ...
    tab1, tab2, tab3 = st.tabs([
        "这个工具可以用来做什么？ / What this tool does",
...
However, this view often ignores two important facts:
...
Although Soul Gems may appear cheaper per attempt, failures can cause item downgrade and repeated upgrading, leading to total resource consumption far higher than intuitive expectations.

Therefore, the truly important question is not:
...
Another common experience-based belief is:

> “Use only Soul Gems for lucky items, and Bless only up to +2 for non-lucky items.”

However, under different servers and market environments:

- the price ratio between Bless Gems and Soul Gems may be very close;
- some new servers may even experience price inversion;
- even experienced players often struggle to determine *when* Bless Gems are actually worth using.

Similarly, when +0 items and +7 / +9 finished items coexist in the market, players often rely purely on intuition or community experience to decide:

> “Is it more economical to upgrade manually or buy the finished item?”

One core purpose of this tool is therefore to convert long-standing player experience and intuition into a quantitative, reproducible, and verifiable decision-making model.
        """)
...
Players can select an equipment type or manually define the Soul Gem success rate to calculate from +0 to the target level (+7 or +9):

- which stages are better suited for Bless Gems or Soul Gems;
- long-term cost differences between strategies;
- the optimal upgrade path under current parameters.

For players who frequently upgrade or perform bulk upgrades, the tool can significantly reduce average gem consumption.

2. Estimate the theoretical value gap between +0 items and high-level items  
Based on the relative market prices of Bless and Soul Gems, the tool can estimate expected upgrade costs from +0 to +7 or +9, providing a theoretical reference for item pricing.

For example, the theoretical value of a +9 item should consider:

- the base item value;
- the expected resources consumed during upgrading.

However, actual market prices are often lower than the theoretical upgrade cost.

This is because sellers of high-level items usually want to quickly exchange them for cash or gems, creating a buyer-favored market. In contrast, +0 items are often obtained through farming or monster drops, where sellers expect a reasonable return, resulting in a more balanced market.

Thus:

> the actual market price gap between +7 / +9 and +0 items is usually lower than the theoretical upgrade cost.
...
Conversely, if high-level items are overpriced, unavailable, or the player has lower acquisition costs, self-upgrading may be more advantageous.

4. Analyze strategy changes under different server economies  
By adjusting Bless relative cost and observing the **Strategy Switching Curve**, players can analyze:

- when the optimal strategy changes;
- whether Bless Gems are worth using in the current server economy;
- which stages are best for using Bless;
- how upgrade risk changes under different economic conditions.

5. Provide resource allocation references for in-game traders  
For trader-oriented players, the tool can also analyze:

- whether gem prices are reasonable;
- whether item prices deviate from theoretical cost;
- potential opportunities for undervalued purchases, liquidation, hoarding, or bottom-fishing.

For example:

if a gem type is significantly undervalued relative to its actual upgrade value, it may indicate a market opportunity; if overpriced, it may be preferable to reduce usage or wait for the market to correct.

For long-term players, traders, and guild resource managers, the tool supports more rational resource allocation decisions.
        """)
