"""
MU Online +0 to +7 Upgrade Strategy Model

This script evaluates all 64 strategies for upgrading an item from +0 to +7.

Rules:
- +0 to +6: each stage can use either Bless Gem or Soul Gem
- +6 to +7: Soul Gem only
- Bless Gem: 100% success
- Soul Gem:
  +0 failure stays at +0
  +1~+6 failure downgrades by 1 level
...
# 输出文件名
# Output file name
output_file = "strategy_results_plus0_to_plus7.csv"
...
# States: +0 to +7
# +7 为吸收态，+0 到 +6 为暂态
# +7 is the absorbing state; +0 to +6 are transient states
transient_states = list(range(7))  # 0~6
absorbing_state = 7

# 可自由选择祝福/灵魂的阶段：
# Decision stages where Bless or Soul can be selected:
# +0→+1, +1→+2, ..., +5→+6
decision_states = list(range(6))  # 0~5

# +6→+7 必须使用灵魂
# +6→+7 must use Soul
forced_soul_states = [6]
...
def fail_state(i):
    ...
    +1~+6 失败回退 1 级。
    Failure from +1 to +6 downgrades by one level.
    """
    if i == 0:
        return 0
    elif 1 <= i <= 6:
...
def generate_strategies():
    ...
    +6→+7 自动补充为 S。
    The +6→+7 stage is automatically fixed as S.
    """
    strategies = []

    for choices in itertools.product(["B", "S"], repeat=6):
        full_strategy = list(choices) + ["S"]
...
def build_transition_matrix(strategy):
    """
    构建暂态转移矩阵 Q，维度为 7×7。
    Build transient transition matrix Q with dimension 7×7.
    ...
    for i in transient_states:
        ...
        if action == "B":
            ...
            # 如果到达 +7，则进入吸收态，不写入 Q
            # If reaching +7, it is absorbing and not included in Q
...
def build_cost_vectors(strategy):
    ...
    """
    bless_cost_vector = np.zeros(7)
    soul_cost_vector = np.zeros(7)
    total_cost_vector = np.zeros(7)
...
def evaluate_strategy(strategy):
    """
    计算单个策略从 +0 到 +7 的期望消耗。
    Evaluate expected consumption from +0 to +7 for one strategy.
    """
...
if __name__ == "__main__":
    ...
    # ============================================================
    # 参数输出 / Print Parameter Settings
    # ============================================================

    print("=" * 60)
    print("MU Online +0 to +7 Upgrade Strategy Model")
    print("=" * 60)

    print("\nParameter Settings:")

    print(f"Soul success probability (p_soul): {p_soul:.4f}")
    print(f"Soul failure probability      : {q_soul:.4f}")

    print(f"Bless relative cost           : {cost_bless}")
    print(f"Soul relative cost            : {cost_soul}")

    print(f"\nTotal strategies evaluated    : 64")
